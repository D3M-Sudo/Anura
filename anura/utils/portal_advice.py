...
screenshot portal fails because no backend exposes the Screenshot interface.
It detects the current desktop session from ``XDG_CURRENT_DESKTOP`` and returns
translated advice strings that tell the user exactly which package to install.

Example:
    from anura.utils.portal_advice import detect_portal_advice

    advice = detect_portal_advice()
    banner_title = advice.short_message  # One-line for Adw.Banner
    toast_body = advice.long_message     # Detailed for notification
"""
...
class PortalAdvice:
    """Structured advice for a missing screenshot portal backend.

    Attributes:
        desktop: Friendly name of the detected desktop (e.g., "LXQt", "KDE Plasma").
