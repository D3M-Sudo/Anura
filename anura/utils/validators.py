...
def is_safe_url_string(text: str) -> bool:
    """
    Perform basic security checks on a URL string.

    Checks for:
    - Null or empty input
    - Maximum length (2048 chars)
    - Control character injection (0x00-0x1F, 0x7F)
    - Non-ASCII characters (Unicode homograph attacks)

    Args:
        text: The string to validate.

    Returns:
        True if the string passes basic safety checks, False otherwise.
    """
    if text is None:
        return False
