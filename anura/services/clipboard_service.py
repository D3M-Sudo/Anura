...
class ClipboardService(GObject.GObject):
    ...
    def _on_uri_list_bytes(self, data: bytes) -> None:
        ...
        if not file_uri:
            # Mask the URIs in the debug log to avoid credential leakage if any present
            masked_uris = [mask_url(u) for u in uris[:3]]
            logger.debug(f"Anura Clipboard: no file:// URI in list (entries={masked_uris!r}).")
        ...
        try:
            ...
            logger.warning(f"Anura Clipboard: bad file URI {mask_url(file_uri)!r}: {e.message}")
