...
class TestReliabilityEnterprise:
    ...
    def test_tts_network_outage(self, tts_service):
        ...
        with (
            patch("gtts.gTTS.save", side_effect=requests.RequestException("Connection timeout")),
    ...
    def test_ocr_missing_binary(self):
        ...
        with patch("shutil.which", return_value=None), patch("loguru.logger.error") as mock_log:
            ScreenshotService()
            # _configure_tesseract_path (called in __init__) logs the error.
